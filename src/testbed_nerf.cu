...
// Get left bottom corner for each patch, doesn't support importance sampling
inline __device__ Vector2f regnerf_random_image_pos_training(default_rng_t& rng, const Vector2i& resolution, bool snap_to_pixel_centers, uint32_t patch_size)
{
	Vector2f xy = random_val_2d(rng);
	xy = xy.cwiseProduct(resolution.cast<float>() - Vector2f::Constant(float(patch_size - 1))).cast<int>().cwiseMax(0).cwiseMin(resolution - Vector2i::Constant(patch_size)).cast<float>();
	if (snap_to_pixel_centers) {
...
inline __device__ uint32_t image_idx_regnerf(uint32_t base_idx, uint32_t n_patches, uint32_t n_patches_total, uint32_t n_training_images)
{
	return (((base_idx + n_patches_total) * n_training_images) / n_patches) % n_training_images;
}

// performs very bad when lr is high
inline __device__ uint32_t image_idx_regnerf_random(default_rng_t& rng, uint32_t n_training_images)
{
	uint32_t img = random_val(rng) * n_training_images;
	if(img == n_training_images)img -= 1;
	return img;
}
...
__global__ void compute_extra_ray_loss_kernel_train_nerf(
	...
	// Must be same seed as above to obtain the same max_level.
	uint32_t ray_idx = ray_indices_in[i];
	rng.advance(ray_idx * N_MAX_RANDOM_SAMPLES_PER_RAY());
	float max_level = max_level_rand_training ? (random_val(rng) * 2.0f) : 1.0f; // Multiply by 2 to ensure 50% of training is at max level
	...
	for (uint32_t j = 0; j < compacted_numsteps; ++j) {
		...
		local_dL_doutput[1] = 0;
		local_dL_doutput[2] = 0;
...
__global__ void generate_training_patches_regnerf(
	const uint32_t n_patches,
	const uint32_t patch_size,
	const uint32_t n_patches_total,
	default_rng_t rng,
	float* __restrict__ patch_pos, // img, x, y
	const uint32_t n_training_images,
	const TrainingImageMetadata* __restrict__ metadata,
	bool snap_to_pixel_centers
)
{
	...
	rng.advance(i * 4);
	uint32_t img = image_idx_regnerf(i, n_patches, n_patches_total, n_training_images);
	// uint32_t img = image_idx_regnerf_random(rng, n_training_images);
	Eigen::Vector2i resolution = metadata[img].resolution;
	Vector2f xy = regnerf_random_image_pos_training(rng, resolution, snap_to_pixel_centers, patch_size);
	// if (i == 0)
	// {
	// 	printf("[%.3f, %.3f]\n", xy.x(), xy.y());
	// }
	patch_pos[3 * i + 0] = img;
	patch_pos[3 * i + 1] = xy.x();
	patch_pos[3 * i + 2] = xy.y();
	// debug
	// if(i == 0)printf("[%d, %.4f, %.4f]\n", img, xy.x(), xy.y());
	// printf("%d ", img);
	// if(i == n_patches - 1)printf("\n");
}
...
// n_patches
// patch_size
// launch format: <<<patch_num, {patch_size, patch_size, 1}, 0, stream>>>
__global__ void generate_training_sample_patches_regnerf(
	const uint32_t n_patches, // new
	const uint32_t patch_size, // new
	BoundingBox aabb,
	const uint32_t max_samples,
	default_rng_t rng,
	float* __restrict__ patch_pos, // new, img, x, y
	uint32_t* __restrict__ patch_counter, // new
	uint32_t* __restrict__ numsteps_counter,
	uint32_t* __restrict__ patch_indices_out, // new
	Ray* __restrict__ rays_out_unnormalized,
	uint32_t* __restrict__ numsteps_out,
	PitchedPtr<NerfCoordinate> coords_out,
	const uint32_t n_training_images,
...
)
{
	...
	const uint32_t i = threadIdx.x + patch_size * (threadIdx.y + patch_id * patch_size);
	patch_pos += patch_id * 3;
	uint32_t img = patch_pos[0];
	Eigen::Vector2i resolution = metadata[img].resolution;
	rng.advance(i * N_MAX_RANDOM_SAMPLES_PER_RAY());
	Vector2f xy;
	xy.x() = patch_pos[1] + float(threadIdx.x) / float(resolution.x());
	xy.y() = patch_pos[2] + float(threadIdx.y) / float(resolution.y());
	size_t pix_idx = pixel_idx(xy, resolution, 0);
	if (read_rgba(xy, resolution, metadata[img].pixels, metadata[img].image_data_type).x() < 0.0f) {
		return;
	}
	...
	// debug: check whether the thread has reached here
	// if (threadIdx.x == 0 && threadIdx.y == 0)
		// printf("block %d\n", patch_id);
	uint32_t laneid;
	uint32_t warpid = (threadIdx.x + threadIdx.y * patch_size) / (patch_size * patch_size);
	asm("mov.u32 %0, %%laneid;" : "=r"(laneid));
	// asm("mov.u32 %0, %%warpid;" : "=r"(warpid));
	uint32_t numsteps = j;
	uint32_t base = j;
	__shared__ uint32_t base_tmp[8];
	// special for patch_size = 4
	if (patch_size == 4)
	{
		uint32_t tp = __shfl_up_sync(0x0000ffffu, base, 1, 16);
		if (laneid >= 1) base += tp;
		tp = __shfl_up_sync(0x0000ffffu, base, 2, 16);
		if (laneid >= 2) base += tp;
		tp = __shfl_up_sync(0x0000ffffu, base, 4, 16);
		if (laneid >= 4) base += tp;
		tp = __shfl_up_sync(0x0000ffffu, base, 8, 16);
		if (laneid >= 8) base += tp;
	}
	else // patch_size == 8 or 16
	{
		uint32_t tp = __shfl_up_sync(0xffffffffu, base, 1);
		if (laneid >= 1) base += tp;
		tp = __shfl_up_sync(0xffffffffu, base, 2);
		if (laneid >= 2) base += tp;
		tp = __shfl_up_sync(0xffffffffu, base, 4);
		if (laneid >= 4) base += tp;
		tp = __shfl_up_sync(0xffffffffu, base, 8);
		if (laneid >= 8) base += tp;
		tp = __shfl_up_sync(0xffffffffu, base, 16);
		if (laneid >= 16) base += tp;
		if (laneid == 31) base_tmp[warpid] = base;
		__syncthreads();
		for (int c0(0); c0 < warpid; ++c0) base += base_tmp[c0];
	}
	if (threadIdx.x == patch_size - 1 && threadIdx.y == patch_size - 1)
	{
		uint32_t prev_base = atomicAdd(numsteps_counter, base);
		base_tmp[1] = prev_base;
		// discard this patch
		if (prev_base + base > max_samples) base_tmp[2] = 1;
		else
		{
			uint32_t pidx = atomicAdd(patch_counter, 1);
			patch_indices_out[pidx] = patch_id;
			// printf("sample: [patch_id, prev, base]: [%d, %d, %d]\n", patch_id, prev_base, base);
			base_tmp[0] = pidx;
			base_tmp[2] = 0;
		}
	}
	__syncthreads();
	if (base_tmp[2]) return;

	uint32_t patch_idx = base_tmp[0];
	base = base + base_tmp[1] - numsteps;
	coords_out += base;
	// patch_indices_out[patch_idx] = patch_id;
	uint32_t ray_idx = threadIdx.x + patch_size * (threadIdx.y + patch_idx * patch_size);
	rays_out_unnormalized[ray_idx] = ray_unnormalized;
	numsteps_out[ray_idx * 2 + 0] = numsteps;
	numsteps_out[ray_idx * 2 + 1] = base;
...
// regnerf loss, need to calculate the ray and take samples in the same way as it is in function ```generate_training_sample_patches_regnerf```
// launch format: <<<valid_patch_num, {patch_size, patch_size, 1}, 0, stream>>>
// Has mem leak!!!!!!!!!!!!!!!!!!!!!
__global__ void compute_loss_kernel_train_patches_regnerf(
	const uint32_t n_rays,
	const uint32_t n_patches, // new
	const uint32_t patch_size, // new
	BoundingBox aabb,
	...
	float* __restrict__ patch_pos, // new, img, x, y
	const uint32_t max_samples_compacted,
	const uint32_t* __restrict__ patches_counter,
	float loss_scale,
	int padded_output_width,
	const float* __restrict__ envmap_data,
	float* __restrict__ envmap_gradient,
	...
	const tcnn::network_precision_t* network_output,
	uint32_t* __restrict__ numsteps_counter,
	const uint32_t* __restrict__ patch_indices_in, // new
	const Ray* __restrict__ rays_in_unnormalized,
	uint32_t* __restrict__ numsteps_in,
	PitchedPtr<const NerfCoordinate> coords_in,
	PitchedPtr<NerfCoordinate> coords_out,
	...
	// the same names in function generate_training_sample_patches_regnerf
	const uint32_t patch_idx = blockIdx.x;
	if (patch_idx >= *patches_counter) return;
	const uint32_t patch_id = patch_indices_in[patch_idx];
	// debug
	if (threadIdx.x == 0 && threadIdx.y == 0)
	{
		// printf("loss: [block, patch]: [%d, %d]\n", patch_idx, patch_id);
	}
	const uint32_t ray_idx = threadIdx.x + patch_size * (threadIdx.y + patch_idx * patch_size);
	const uint32_t i = threadIdx.x + patch_size * (threadIdx.y + patch_id * patch_size);

	// grab the number of samples for this ray, and the first sample
	uint32_t numsteps = numsteps_in[ray_idx * 2 + 0];
	...
	// Must be same seed as above to obtain the same background color, which was the "i" in patch patch_id in fucntion ```generate_training_sample_patches_regnerf``` 
	// patch samples
	patch_pos += patch_id * 3;
	uint32_t img = patch_pos[0];
	Eigen::Vector2i resolution = metadata[img].resolution;
	...
	Vector3f dir;
	if (envmap_data) {
		dir = rays_in_unnormalized[ray_idx].d.normalized();
		envmap_value = read_envmap(envmap_data, envmap_resolution, dir);
		background_color = envmap_value.head<3>() + background_color * (1.0f - envmap_value.w());
	}
	...
	uint32_t compacted_base = atomicAdd(numsteps_counter, compacted_numsteps); // first entry in the array is a counter
	compacted_numsteps = min(max_samples_compacted - min(max_samples_compacted, compacted_base), compacted_numsteps);
	numsteps_in[ray_idx * 2 + 0] = compacted_numsteps;
	numsteps_in[ray_idx * 2 + 1] = compacted_base;
	if (compacted_numsteps == 0) {
		return;
	}
	...
	float target_depth = rays_in_unnormalized[ray_idx].d.norm() * ((depth_supervision_lambda > 0.0f && metadata[img].depth) ? read_depth(xy, resolution, metadata[img].depth) : -1.0f);
	LossAndGradient lg_depth = loss_and_gradient(Array3f::Constant(target_depth), Array3f::Constant(depth_ray), depth_loss_type);
	float depth_loss_gradient = target_depth > 0.0f ? depth_supervision_lambda * lg_depth.gradient.x() : 0;
...
void Testbed::NerfCounters::prepare_for_training_steps(cudaStream_t stream, bool sample_patches, uint32_t patch_size) {
	numsteps_counter.enlarge(1);
	numsteps_counter_compacted.enlarge(1);
	numsteps_counter_extra.enlarge(1);
	numsteps_counter_compacted_extra.enlarge(1);
	if (sample_patches)
	{
		patches_per_batch = rays_per_batch / (patch_size * patch_size);
		rays_per_batch = patch_size * patch_size * patches_per_batch;
		printf("Patches per batch: %d\n", patches_per_batch);
	}
	...
	CUDA_CHECK_THROW(cudaMemsetAsync(numsteps_counter_extra.data(), 0, sizeof(uint32_t), stream)); // clear the counter in the first slot
	CUDA_CHECK_THROW(cudaMemsetAsync(numsteps_counter_compacted_extra.data(), 0, sizeof(uint32_t), stream)); // clear the counter in the first slot
}
...
float Testbed::NerfCounters::update_after_training(uint32_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
	...
	// calc extra rays per batch
	numsteps_counter_extra.copy_to_host(counter_cpu);
	numsteps_counter_compacted_extra.copy_to_host(compacted_counter_cpu);
	measured_batch_size_extra = 0;
	measured_batch_size_before_compaction_extra = 0;

	if (counter_cpu[0] != 0 && compacted_counter_cpu[0] != 0)
	{
		measured_batch_size_before_compaction_extra = counter_cpu[0];
		measured_batch_size_extra = compacted_counter_cpu[0];

		rays_per_batch_extra = (uint32_t)((float)rays_per_batch_extra * (float)target_batch_size / (float)measured_batch_size_extra);
		rays_per_batch_extra = std::min(next_multiple(rays_per_batch_extra, tcnn::batch_size_granularity), 1u << 18);
	}

	counter_cpu[0] = 0;
	compacted_counter_cpu[0] = 0;
...
void Testbed::train_nerf(uint32_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
	...
	{
		m_nerf.training.patch_size = 1 << m_nerf.training.log_patch_size;
	}
	m_nerf.training.counters_rgb.prepare_for_training_steps(stream, m_nerf.training.sample_patches, m_nerf.training.patch_size);
	...
	if (m_nerf.training.sample_patches)
	{
		train_regnerf_step(target_batch_size, m_nerf.training.counters_rgb, m_nerf.training.patch_size, stream);
	}
	else
	{
	...
	if (m_nerf.training.train_extra_ray)
	{
		train_extra_nerf_step(target_batch_size, m_nerf.training.counters_rgb, stream);
		m_trainer->optimizer_step(stream, LOSS_SCALE);
	}
...
void Testbed::train_nerf_step(uint32_t target_batch_size, Testbed::NerfCounters& counters, cudaStream_t stream) {
	...
	>(
		...
		max_samples,
		// extra rays
		counters.rays_per_batch_extra,
		counters.rays_per_batch_extra,
		counters.rays_per_batch_extra * 2
	);
	...
	// extra rays
	uint32_t* extra_ray_indices = std::get<15>(scratch);
	Ray* extra_rays_unnormalized = std::get<16>(scratch);
	uint32_t* extra_numsteps = std::get<17>(scratch);
...
// generate extra rays and train their densities
void Testbed::train_extra_nerf_step(uint32_t target_batch_size, Testbed::NerfCounters& counters, cudaStream_t stream) {
	const uint32_t padded_output_width = m_network->padded_output_width();
	const uint32_t max_samples = target_batch_size * 16; // Somewhat of a worst case
	const uint32_t floats_per_coord = sizeof(NerfCoordinate) / sizeof(float) + m_nerf_network->n_extra_dims();
	const uint32_t extra_stride = m_nerf_network->n_extra_dims() * sizeof(float); // extra stride on top of base NerfCoordinate struct
	...
	// TODO: C++17 structured binding
	uint32_t* ray_indices_extra = std::get<0>(scratch);
	Ray* rays_unnormalized_extra = std::get<1>(scratch);
	uint32_t* numsteps_extra = std::get<2>(scratch);
	float* coords = std::get<3>(scratch);
	float* max_level = std::get<4>(scratch);
	network_precision_t* mlp_out = std::get<5>(scratch);
	network_precision_t* dloss_dmlp_out = std::get<6>(scratch);
	...
	uint32_t max_inference;
	if (counters.measured_batch_size_before_compaction_extra == 0) {
		counters.measured_batch_size_before_compaction_extra = max_inference = max_samples;
	}
	else {
		max_inference = next_multiple(std::min(counters.measured_batch_size_before_compaction_extra, max_samples), tcnn::batch_size_granularity);
	}
	...
	if (m_training_step == 0) {
		counters.n_rays_total_extra = 0;
	}

	counters.n_rays_total_extra += counters.rays_per_batch_extra;

	CUDA_CHECK_THROW(cudaMemsetAsync(ray_counter, 0, sizeof(uint32_t), stream));

	linear_kernel(generate_extra_training_samples_nerf, 0, stream,
		counters.rays_per_batch_extra,
		m_aabb,
		max_inference,
		m_rng,
		ray_counter,
		counters.numsteps_counter_extra.data(),
		ray_indices_extra,
		rays_unnormalized_extra,
		numsteps_extra,
		PitchedPtr<NerfCoordinate>((NerfCoordinate*)coords, 1, 0, extra_stride),
		s_coord, // regularized t, mid point of sample section
		m_nerf.density_grid_bitfield.data(),
	...
	linear_kernel(compute_extra_ray_loss_kernel_train_nerf, 0, stream,
		counters.rays_per_batch_extra,
		m_aabb,
		//n_rays_total,
		m_rng,
		...
		//m_nerf.training.dataset.metadata_gpu.data(),
		mlp_out,
		counters.numsteps_counter_compacted_extra.data(),
		ray_indices_extra,
		rays_unnormalized_extra,
		numsteps_extra,
		PitchedPtr<const NerfCoordinate>((NerfCoordinate*)coords, 1, 0, extra_stride),
	...
	fill_rollover_and_rescale<network_precision_t> << <n_blocks_linear(target_batch_size * padded_output_width), n_threads_linear, 0, stream >> > (
		target_batch_size, padded_output_width, counters.numsteps_counter_compacted_extra.data(), dloss_dmlp_out
		);
	fill_rollover<float> << <n_blocks_linear(target_batch_size * floats_per_coord), n_threads_linear, 0, stream >> > (
		target_batch_size, floats_per_coord, counters.numsteps_counter_compacted_extra.data(), (float*)coords_compacted
		);
	fill_rollover<float> << <n_blocks_linear(target_batch_size), n_threads_linear, 0, stream >> > (
		target_batch_size, 1, counters.numsteps_counter_compacted_extra.data(), max_level_compacted
		);

	{
		auto ctx = m_network->forward(stream, compacted_coords_matrix, &compacted_rgbsigma_matrix, false, false);
		m_network->backward(stream, *ctx, compacted_coords_matrix, compacted_rgbsigma_matrix, gradient_matrix, nullptr, false, EGradientMode::Overwrite);
	}
...
// generate geometry regularization rays and train their densities
void Testbed::train_regnerf_step(uint32_t target_batch_size, Testbed::NerfCounters& counters, uint32_t patch_size, cudaStream_t stream) {
	const uint32_t padded_output_width = m_network->padded_output_width();
	const uint32_t max_samples = target_batch_size * 16; // Somewhat of a worst case
	const uint32_t floats_per_coord = sizeof(NerfCoordinate) / sizeof(float) + m_nerf_network->n_extra_dims();
	const uint32_t extra_stride = m_nerf_network->n_extra_dims() * sizeof(float); // extra stride on top of base NerfCoordinate struct
	...
	auto scratch = allocate_workspace_and_distribute<
		...
		float, // coords_gradient
		float, // max_level_compacted
		uint32_t, // new, patch_counter
		float // patch_pos
	>(
		stream, &alloc,
		counters.patches_per_batch,
		counters.rays_per_batch,
		counters.rays_per_batch * 2,
		max_samples * floats_per_coord,
		max_samples,
		...
		target_batch_size,
		1,
		3 * counters.patches_per_batch
	);

	// TODO: C++17 structured binding
	uint32_t* patch_indices = std::get<0>(scratch);
	Ray* rays_unnormalized = std::get<1>(scratch);
	uint32_t* numsteps = std::get<2>(scratch);
	float* coords = std::get<3>(scratch);
	float* max_level = std::get<4>(scratch);
	...
	float* coords_gradient = std::get<8>(scratch);
	float* max_level_compacted = std::get<9>(scratch);
	uint32_t* patches_counter = std::get<10>(scratch);
	float* patch_pos = std::get<11>(scratch);
	...
	if (m_training_step == 0) {
		...
		m_nerf.training.counters_rgb.n_patches_total = 0;
	}

	uint32_t n_rays_total = m_nerf.training.counters_rgb.n_rays_total;
	m_nerf.training.counters_rgb.n_rays_total += counters.rays_per_batch;
	m_nerf.training.n_rays_since_error_map_update += counters.rays_per_batch;

	uint32_t n_patches_total = m_nerf.training.counters_rgb.n_patches_total;
	m_nerf.training.counters_rgb.n_patches_total += counters.patches_per_batch;
	...
	linear_kernel(generate_training_patches_regnerf, 0, stream,
		counters.patches_per_batch,
		patch_size,
		n_patches_total,
		m_rng,
	...
	CUDA_CHECK_THROW(cudaMemsetAsync(patches_counter, 0, sizeof(uint32_t), stream));
	m_rng.advance(counters.patches_per_batch);
	cudaStreamSynchronize(stream);
	printf("Generate samples!\n");
	generate_training_sample_patches_regnerf<<<{counters.patches_per_batch, 1, 1}, {patch_size, patch_size, 1}, 0, stream>>>
	(
		counters.patches_per_batch,
		patch_size,
		m_aabb,
	...
	cudaStreamSynchronize(stream);
	printf("Compute loss!\n");
	uint32_t patches_counter_cpu;
	cudaMemcpy(&patches_counter_cpu, patches_counter, sizeof(uint32_t), cudaMemcpyDeviceToHost);
	printf("Valid patches num: %d\n", patches_counter_cpu);
	uint32_t* patch_indices_cpu = new uint32_t[counters.patches_per_batch];
	cudaMemcpy(patch_indices_cpu, patch_indices, sizeof(uint32_t) * counters.patches_per_batch, cudaMemcpyDeviceToHost);
	for (int c0(0);c0 < 10; ++c0)
	{
		printf("[%d %d]\n", c0, patch_indices_cpu[c0]);
	}
	// notice that we must use the same ray generator as in func generate_training_samples_regnerf instead of generate_training_samples_nerf
	compute_loss_kernel_train_patches_regnerf<<<{counters.patches_per_batch, 1, 1}, {patch_size, patch_size, 1}, 0, stream>>>
	(
		counters.rays_per_batch,
		counters.patches_per_batch, // new
		patch_size, // new
		m_aabb,
		n_rays_total,
		m_rng,
