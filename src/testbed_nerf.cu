...
__global__ void generate_training_samples_regnerf(
	const uint32_t n_rays,
	...
	// rng.advance(i * N_MAX_RANDOM_SAMPLES_PER_RAY());
	// uint32_t img = image_idx_regnerf(i, n_patches, n_rays_total, n_training_images);
	// uint32_t img = image_idx_regnerf_random(rng, n_training_images);
	// uint32_t img = image_idx(i, n_rays, n_rays_total, n_training_images, cdf_img);
	// Eigen::Vector2i resolution = metadata[img].resolution;
	...
	// patch sampling
	uint32_t patch_pixels = patch_size * patch_size;
	uint32_t patch_id = i / patch_pixels;
	uint32_t x_offset = i % patch_size;
	uint32_t y_offset = (i / patch_size) % patch_size;
	...
	rng.advance(i * N_MAX_RANDOM_SAMPLES_PER_RAY());
	Vector2f xy;
	xy.x() = patch_pos[1] + float(x_offset) / float(resolution.x());
	xy.y() = patch_pos[2] + float(y_offset) / float(resolution.y());
	// xy = random_val_2d(rng);
	// xy = nerf_random_image_pos_training(rng, resolution, snap_to_pixel_centers, cdf_x_cond_y, cdf_y, cdf_res, img);
	// end
	// Negative values indicate masked-away regions
	size_t pix_idx = pixel_idx(xy, resolution, 0);
	if (read_rgba(xy, resolution, metadata[img].pixels, metadata[img].image_data_type).x() < 0.0f) {
	...
	// maybe we can do some reduction here since sicne we regard a patch as a group
	// 
	uint32_t numsteps = j;
	uint32_t base = atomicAdd(numsteps_counter, numsteps);	 // first entry in the array is a counter
	if (base + numsteps > max_samples) {
...
__global__ void compute_loss_kernel_train_regnerf(
	const uint32_t n_rays,
	const uint32_t n_patches, // new
	const uint32_t patch_size, // new
	BoundingBox aabb,
	...
	// patch samples
	uint32_t patch_pixels = patch_size * patch_size;
	uint32_t patch_id = ray_idx / patch_pixels;
	uint32_t x_offset = ray_idx % patch_size;
	uint32_t y_offset = (ray_idx / patch_size) % patch_size;
	patch_pos += patch_id * 3;
	uint32_t img = patch_pos[0];
	// uint32_t img = image_idx(i, n_rays, n_rays_total, n_training_images, cdf_img);
	Eigen::Vector2i resolution = metadata[img].resolution;
...
void Testbed::Nerf::Training::Counters::prepare_for_training_steps(cudaStream_t stream, bool sample_patches , uint32_t patch_size) {
...
void Testbed::train_nerf(uint32_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
	...
	if (m_nerf.training.sample_patches)
	{
		train_regnerf_step(
			...
			m_nerf.training.counters_rgb.patches_per_batch, // new
			m_nerf.training.patch_size, // new
...
void Testbed::train_regnerf_step(uint32_t target_batch_size, uint32_t n_rays_per_batch, uint32_t n_patches_per_batch, uint32_t patch_size, uint32_t* counter, uint32_t* compacted_counter, float* loss, cudaStream_t stream) {
	const uint32_t padded_output_width = m_network->padded_output_width();
	const uint32_t max_samples = target_batch_size * 16; // Somewhat of a worst case
	const uint32_t floats_per_coord = sizeof(NerfCoordinate) / sizeof(float) + m_nerf_network->n_extra_dims();
	const uint32_t extra_stride = m_nerf_network->n_extra_dims() * sizeof(float); // extra stride on top of base NerfCoordinate struct
	...
	m_nerf.training.counters_rgb.n_patches_total += n_patches_per_batch;
	...
	m_rng.advance(n_patches_per_batch);
	cudaStreamSynchronize(stream);
	printf("Generate samples!\n");
	generate_training_sample_patches_regnerf<<<{n_patches_per_batch, 1, 1}, {patch_size, patch_size, 1}, 0, stream>>>
	(
		n_patches_per_batch,
		patch_size,
		m_aabb,
	...
	uint32_t* patch_indices_cpu = new uint32_t[n_patches_per_batch];
	cudaMemcpy(patch_indices_cpu, patch_indices, sizeof(uint32_t) * n_patches_per_batch, cudaMemcpyDeviceToHost);
	...
	compute_loss_kernel_train_patches_regnerf<<<{n_patches_per_batch, 1, 1}, {patch_size, patch_size, 1}, 0, stream>>>
	(
		n_rays_per_batch,
		n_patches_per_batch, // new
		patch_size, // new
		m_aabb,
		n_rays_total,
		m_rng,
